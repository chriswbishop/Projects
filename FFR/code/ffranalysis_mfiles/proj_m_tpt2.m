function [s1,s2,s3,phase_results] = proj_m_tpt2(sx,sy,xtitle,ax,output)
%function [p,pct2,pr,s1,s2,s3,phase_results] =%proj_m_tpt2(sx,sy,xtitle,ax,output)
% slightly revised April 2008 by Clinard.
% annotates figure with results; divided code into cells
%
...
r = (sqrt(sumc*sumc + sums*sums))/n;            % phase coherence
...
if (output == 1)
% h1 = figure;
%  h2 = axes('position', [.1 .1 .6 .7]); % for one large plot
%   h2 = axes('position', [.1 .2 .4 .5]); % as subplot on left side
 h4 = axes('position', [.15 .1 .2 .2]); % for use with Meeg2ascii.m for Project M
 hold on;
 axis equal;
 ...
 axis([-ax +ax -ax +ax]);
 title(sprintf('%s polar',xtitle),'Interpreter','none');
 xlabel('real (\muV)'); ylabel('imaginary');
 plot(zz,'r','LineWidth',2);     % ellipse from t-squared;  
 plot(zct,'g','LineWidth',2);    % circular t-squared's circle
 plot(ccc,'LineWidth', 2);                 % line from (0,0) to mean
 plot(-1:.02:1,xa);         % plot x-axis line of quadrant divisions
 plot(ya,-1:.02:1);         % plot y-axis line, Problem with displaying results
 plot(zz,'r','LineWidth',2);     % ellipse from t-squared;  
 plot(zct,'g','LineWidth',2);    % circular t-squared's circle
 hold off;
end
...
%% Make a structure to sent to Meeg2ascii_singlefile.m
