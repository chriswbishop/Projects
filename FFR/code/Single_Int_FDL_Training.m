...
filename = [num2str(args.subject_id{:}) '_FDL_Train', ...
       num2str(frequency) 'Hz_b', num2str(args.block_number{:})];
pause(1.0);
...
%       [tone_final] = make_AM_FDL_tones_2freq_4FFR(fs, carrier_freq1, carrier_freq2, calibrate, save_wav)
      ...
      [tone] = make_AM_FDL_tones_2freq_4FFR(args.samprate, frequency, carrier_freq2, 0, 0);
...
%       [tone noise]  = make_tone_plus_noise(frequency, catch_vals(catch_trials));  %%%%%%%%% 6/13/2012 
      ...
      [tone] = make_AM_FDL_tones_2freq_4FFR(args.samprate, carrier_freq2, carrier_freq2, 0, 0);
  ...
  elseif calibrate == 1
   ...
   if calibrate < 1
    temp_filename = [ args.path 'temp_behavior\ProjZA' filename '_TEMP_' ];
    save(temp_filename)
   end
