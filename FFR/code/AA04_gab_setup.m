    ...
    for c=1:length(conds)
        for r=1:reps            
            ...
            if strcmpi(SID, 'KM') && conds(c)==1
                tfname=fullfile(studyDir, SID, 'eeg', sprintf([SID '-AA04-%01d-%02d.cnt'], c, r+1));
            else
                tfname=fullfile(studyDir, SID, 'eeg', sprintf([SID '-AA04-%01d-%02d.cnt'], c, r));
            end % switch                   
            
            % Append to file names
            fnames{(c-1)*length(conds) + r}=tfname;
            onames{(c-1)*length(conds) + r}=[tfname(1:end-4) '_Cz-LE' tfname(end-3:end)];
            %   No memory mapping (memmapnames are empty)
            memmapnames{(c-1)*length(conds) + r}='';
