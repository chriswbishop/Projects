function [LAC RAC ALAG VCOD COND LJIT UJIT NSTIM NBREAKS MINCHUNK COFFSET RESP_NSTIM RESP_NGROUP pRESP_NGROUP pRESP_NSTIM LEADONLY LAGONLY GMC P]=PEABR_exp01A_analysis(P)
%% DESCRIPTION:
%
%
...
N=unique(NSTIM(IND)); % Only grab stimuli that were not controls

%% CORRECT GROUP NUMBER RESPONSE PROPORTION
for n=1:length(N)
    data(n)=(length(find(COND~=201 & COND~=202 & COND~=203 & NSTIM==N(n) & (RESP_NGROUP==NBREAKS+1)))./length(find(COND~=201 & COND~=202 & COND~=203 & NSTIM==N(n))));
end % for n=1: ... 
