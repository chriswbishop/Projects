function [LAC RAC ALAG VCOD COND NRESP SRESP NRT SRT STIME X Y VOFF PCOUNT PTOTAL S M PTIME]=MSPE_fMRI_read(P)
%% DESCRIPTION:
%
...
%% INPUT CHECK
% if ~exist('CONCAT', 'var') || isempty(CONCAT), CONCAT=0; end 
% if ~exist('NSCAN', 'var') || isempty(NSCAN), NSCAN=[]; end % don't pad scans by default
...
%% READ LOG FILE
for f=1:size(P,1)
    % reset counter, but only if we aren't concatenating sessions
%     if ~CONCAT
        pcount=0;
%     end % concat
    
    % Open file
    ptr=fopen(deblank(P(f,:)),'r');
    display(P(f,:));
    ...
    while isstr(nl)
        ...
        if strcmp(ET,'Sound')
            ...
            PCOUNT(length(COND))=pcount; 
        end % Sound
        
        if strcmp(ET,'Pulse')
            pcount=pcount+1;  
            PTIME(pcount)=str2num(TIME);
        end % strcmp(ET, 'Pulse'); 
        ...
        if strcmp(ET, 'Manual') && ~isempty(strfind(nl, ';'))
            %% READS IN X/Y COORDINATES FOR EARLIER POINTING TASKS
            %% PUBLISHED IN BISHOP et al. CURRENT BIOLOGY (2010)
            [x rmd]=strtok(COD, ';');
            [y rmd]=strtok(rmd, ';');
            
            X(length(LAC))=str2num(x);
            Y(length(LAC))=str2num(y);        
        elseif strcmp(ET, 'Manual') && isempty(strfind(nl,';'))
            %% GRABS THE MATFILE USED
            M{end+1}=COD; 
        end % Manual
